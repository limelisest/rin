...
import type {Store} from './nerve.js';
import type {ChatConfig} from './chat/types.js';
import type {AttentionConfig, AttentionRecord, AttentionState, AttentionGroup, AttentionRead, AttentionSend, AttentionSender, AttentionSendInput} from './attention-types.js';
import {readFileSync} from 'node:fs';
import {createHash} from 'node:crypto';
import {classifyStoredMessage, createAttentionState, normalizeAttentionState, enqueueAttention, prepareDueBatch, completeEmittingBatch} from './attention.js';

const hash = (value: string) => createHash('sha256').update(value).digest('hex');
const keyChatId = (key: string) => key.slice(key.indexOf(':') + 1);

export class AttentionService {
 declare config: AttentionConfig; declare store:Store; declare db: DatabaseSync; declare sendImpl?:AttentionSender; declare owners:Set<string>; declare ignored:Set<string>; declare mirrors:Set<string>;
  constructor(config: AttentionConfig, store: Store, {send}: {send?:AttentionSender} = {}) {
    this.config = config;
    this.store = store;
    this.db = store.db;
    this.sendImpl = send;
    ...
    if (config.channels !== undefined && (!config.channels || typeof config.channels !== 'object' || Array.isArray(config.channels) || Object.values(config.channels).some(channel=>
      ...
      (['idleDelayMs','maxDelayMs'] as const).some(key=>channel[key] !== undefined && (!Number.isSafeInteger(channel[key]) || channel[key] < 1)) ||
  ...
  state() { return normalizeAttentionState(JSON.parse(this.db.prepare('SELECT state FROM attention_state WHERE id=1').get()!.state as string)); }
  save(state: AttentionState) { this.db.prepare('UPDATE attention_state SET state=? WHERE id=1').run(JSON.stringify(state)); }
  excluded(record: AttentionRecord) {
  ...
  accept(input: AttentionRecord) {
    if (input?.bot || input?.authorBot || input?.role && input.role !== 'user') return {inserted:false, ignored:true};
    for (const name of ['id','messageId','platformInstance','chatKey','userId','receivedAt'] as const) {
    ...
    return this.transaction(()=>{
      ...
      for (const row of notified) {
        ...
        if (Array.isArray(payload.messageIds)) {
          ...
          for (const item of state.pending) if ((payload.groups || []).some((group: AttentionGroup) =>
  ...
  read({chatKey,limit=50,before,markViewed=true,attentionMode,attentionForMs=300000}: AttentionRead = {}) {
    ...
    const messages = rows.reverse().map(row=>{
      const {disposition,...message} = JSON.parse(row.record as string) as AttentionRecord;
      return message;
    });
  ...
  async send({id,chatKey,text,replyTo,awaitingReply=false,awaitingReplyMs=3600000}: AttentionSend = {}) {
    ...
    const record = JSON.parse(raw.record as string);
    ...
    if (existing) {
      ...
      return {id,state:existing.state,deduplicated:true,...(existing.result ? JSON.parse(existing.result as string) : {})};
    }
  ...
  async discordSender(instance: string): Promise<AttentionSender> {
    const config: ChatConfig = JSON.parse(readFileSync(this.config.chatConfig,'utf8'));
    ...
    return async ({id,chatId,text,replyTo}: AttentionSendInput) => rest.post(Routes.channelMessages(chatId),{body:{
