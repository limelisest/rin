...
import type {Exec, ExecOptions, ExecResult, Executable} from './types.js';
interface PlatformOptions {platform?: NodeJS.Platform; arch?: string; libc?: 'gnu' | 'musl';}
interface ProductOptions extends PlatformOptions {
 products?: string[]; home?: string; userHome?: string; node?: string; npmCli?: string; run?: Exec;
 download?: (url: string, destination: string) => Promise<unknown>; exists?: (path: string) => Promise<boolean>;
 osRelease?: Record<string,string>;
}
interface HistoryOptions extends PlatformOptions {
 home?: string; codexHome?: string; run?: Exec; download?: (url: string, destination: string) => Promise<unknown>;
 exists?: (path: string) => Promise<boolean>; verify?: (path: string, expected: string) => Promise<unknown>;
 makeLink?: typeof symlink; codexCommand?: string | Executable;
}
interface ProductResult {product: string; status: 'installed' | 'existing'; command?: string; path?: string; source?: string;}
interface MCPTransport {command?: string; args?: string[];}
interface MCPConfiguration extends MCPTransport {transport?: MCPTransport;}
...
import {codexCommand as resolveCodex,findNpmCli,run as coreRun} from './core.js';
...
export async function downloadFile(url: string,destination: string,{fetchImpl=globalThis.fetch}={}){
...
function assertOk(result: ExecResult,label: string){
...
function parseOsRelease(text: string){
  return Object.fromEntries(String(text).split(/\r?\n/).map(line=>line.match(/^([A-Z_]+)=(.*)$/)).filter((match): match is RegExpMatchArray => match !== null).map(([,key,value])=>[key,value.replace(/^['"]|['"]$/g,'')]));
}
function supportedLinux(os: Record<string,string>){
...
function fffAsset({platform=process.platform,arch=process.arch,libc}: PlatformOptions={}){
  const normalized=normalizeArch(arch);
  const family=platform==='linux'?(libc??((process.report?.getReport() as {header?: {glibcVersionRuntime?: string}} | undefined)?.header?.glibcVersionRuntime?'gnu':'musl')):null;
  const key=platform==='linux'?`${platform}-${normalized}-${family}`:`${platform}-${normalized}`;
  const asset=key in FFF.assets ? FFF.assets[key as keyof typeof FFF.assets] : undefined;
  return asset?{...asset,key,url:`${FFF.base}/${asset.name}`} : null;
}
export function historySupport(options: PlatformOptions={}){
  try{
  ...
  }catch(error){return {supported:false,reason:errorMessage(error)};}
}
export async function installProducts({products,home,userHome=homedir(),platform=process.platform,arch=process.arch,node=process.execPath,npmCli,run=execCommand,download=downloadFile,exists=pathExists,osRelease}: ProductOptions={}){
  ...
  let linuxKind: 'deb' | 'rpm' | undefined,linuxArch: 'x64' | 'arm64' | undefined;
  ...
  for(const product of requested){
    ...
    if (!linuxKind || !linuxArch) throw new Error('Linux package target was not resolved');
...
export async function installHistoryTool({home,codexHome,platform=process.platform,arch=process.arch,libc,run=execCommand,download=downloadFile,exists=pathExists,verify=verifySha256,makeLink=symlink,codexCommand}: HistoryOptions={}){
  ...
  const runGlobalCodex=async(commandArgs: string[],options: ExecOptions={})=>{
  ...
  let conflict = false;
  if(current.code===0){
    let parsed: MCPConfiguration; try{parsed=JSON.parse(current.stdout);}catch{throw new Error('Could not inspect existing session-history MCP configuration');}
  ...
  for(const name of ['sessions','archived_sessions']){
    ...
    }catch(error){if(errorCode(error)==='ENOENT') await makeLink(target,destination,platform==='win32'?'junction':'dir'); else throw error;}
  }
