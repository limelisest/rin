...
function appendMarkdownRange(ranges: Range[], start: number, end: number) {
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start)
        return;
    ranges.push({ start, end });
}
function markdownRangeOverlaps(ranges: Range[], start: number, end: number) {
...
function childMarkdownTokens(token: Token): Token[] {
    const children = [];
    if (Array.isArray(('tokens' in token ? token.tokens : undefined)))
        children.push(...('tokens' in token ? token.tokens ?? [] : []));
    if (Array.isArray(('items' in token ? token.items : undefined)))
        children.push(...('items' in token ? token.items : []));
    return children;
}
function collectNestedMarkdownProtectedRanges(ranges: Range[], blockStart: number, blockRaw: string, tokens: Token[]) {
    let cursor = 0;
...
function markdownLinks(source: string) {
  ...
  const links: (Range & {href: string})[] = [];
  const visit = (tokens: Token[], parentSource: string, base: number) => {
    let cursor = 0;
    for (const token of tokens || []) {
      const raw = safeString(token.raw);
      if (!raw) continue;
...
function attachment(href: string, safeRoots: string[]) {
  let filePath: string;
...
export function outputParts(text: unknown, roots: string[] = []): ChatOutput[] {
...
export function outputFiles(text: unknown, roots: string[] = []) {
  const seen = new Set();
