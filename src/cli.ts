...
export interface CliOptions {home?: string;serviceFactory?: typeof createService;codex?: string;codexHome?: string;writeConfig?: ConfigWriter;prepare?: typeof prepareRelease;switchTo?: typeof switchRelease;ensureMcp?: typeof ensureNerveMcp;activateMcp?: typeof activateNerveMcp}
type Route = {type: 'codex';args: string[]} | {type:'rin';command: string};
...
export async function main(args = process.argv.slice(2), { home = installHome(), serviceFactory = createService, codex = process.env.RIN_CODEX_BIN, codexHome, writeConfig, prepare = prepareRelease, switchTo = switchRelease, ensureMcp, activateMcp = activateNerveMcp }: CliOptions = {}): Promise<number> {
