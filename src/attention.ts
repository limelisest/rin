import type {AttentionRecord, AttentionPolicy, AttentionState, AttentionItem} from './attention-types.js';
import crypto from "node:crypto";

// Attention policy ported from the previous Rin. Persistence and transport live outside this module.

function chatId(chatKey: string) {
  const separator = chatKey.indexOf(":");
  return separator >= 0 ? chatKey.slice(separator + 1) : "";
}

function nextFixedWindow(receivedAt: string, windowMs: number) {
...
export function classifyStoredMessage(message: AttentionRecord, options: AttentionPolicy) {
...
export function createAttentionState(lastMessageId?: string): AttentionState {
  return {
    version: 2,
...
export function normalizeAttentionState(value: AttentionState | null | undefined): AttentionState {
...
export function enqueueAttention(state: AttentionState, item?: AttentionItem) {
  if (!item) return;
...
export function prepareDueBatch(state: AttentionState, nowMs: number, {active=false, chatModes={}, notifiedMessageIds=new Set<string>()}: {active?:boolean; chatModes?:Record<string,string>; notifiedMessageIds?:Set<string>} = {}) {
...
export function completeEmittingBatch(state: AttentionState, batchId: string) {
