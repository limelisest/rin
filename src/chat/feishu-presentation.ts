interface MarkdownToken { type: string; raw?: string; text?: string; href?: string; lang?: string; tokens?: MarkdownToken[]; }
interface PostElement {tag: string; text?: string; href?: string; user_id?: string; style?: string[]; language?: string;}
...
export function normalizeLarkMarkdownListBlocks(text: unknown) {
...
function renderLarkPostBlock(token: MarkdownToken): PostElement[] {
  const type = safeString(token?.type).trim();
...
export function renderLarkPostContent(text: unknown) {
...
export function postData(text: unknown) { return {msg_type:'post',content:JSON.stringify({zh_cn:{content:renderLarkPostContent(normalizeLarkMarkdownListBlocks(text))}})}; }
