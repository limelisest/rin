export interface UsageRecord { id: string; thread: string; turn: string; day: string; input: number; cached: number; cache_write: number; output: number; total: number; model?: string; }
...
export function estimateCodexUsageCost(record: Pick<UsageRecord, 'input' | 'cached' | 'cache_write' | 'output'>, model: string | undefined) {
  const tiers = (CODEX_PRICING.models as Record<string, number[][]>)[model || ''];
