...
function checked<T>(value: T | symbol, ui: UI): T {
...
export async function confirmChoice(message: string, initialValue = false, ui = clack) {
  return checked(await ui.confirm({message,initialValue}), ui);
}

function note(ui: UI, body: string, title: string) {
...
async function promptInstructions(ui: UI) {
...
export async function collectChoices({hasAgents = false, legacy = null, home, agentsPath, ui = clack}: {hasAgents?: boolean; legacy?: LegacyInstallation | null; home?: string; agentsPath?: string; ui?: UI} = {}): Promise<Choices | null> {
