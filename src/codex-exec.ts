...
import type { ExecOptions, ExecResult, ExecEvent } from './runtime-types.js';
...
function uncertain(reason: string) {
  const error: NodeJS.ErrnoException = new Error(`Codex exec ${reason}; outcome uncertain`);
...
export class CodexExec {
  declare command: string[]; declare codexHome: string; declare cwd?: string; declare timeoutMs: number; declare stopped: boolean;
  declare children: Set<{child: ChildProcess; terminate: () => Promise<void>; cancel: () => void}>;
  constructor({ command = ['codex'], codexHome = join(homedir(), '.codex'), cwd, timeoutMs = 1_800_000 }: ExecOptions = {}) {
  ...
  async run(threadId: string, { text }: {text?: string} = {}) {
    ...
    return new Promise<ExecResult>((resolve, reject) => {
      ...
      const signal = (name: NodeJS.Signals) => {
        try { if (grouped && child.pid) process.kill(-child.pid, name); else child.kill(name); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ESRCH') throw error; }
      };
      const terminate = () => killPromise ||= new Promise<void>(done => {
        signal('SIGTERM');
