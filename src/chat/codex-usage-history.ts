import type { UsageRecord } from './codex-usage-pricing.js';
interface FileRecord {inode: string; offset: number; skipping: number; thread: string; provider: string;}
interface DayUsage { requests: number; input_tokens: number; cache_read_tokens: number; output_tokens: number; total_tokens: number; cost_total: number; unpriced_requests: number; }
interface UsageEvent {type: string; timestamp: string; payload?: {response_id?: string; thread_id?: string; turn_id?: string; usage?: Record<string, number>};}
import { CODEX_PRICING, estimateCodexUsageCost } from './codex-usage-pricing.js';
import { mkdir, readdir, open, stat } from 'node:fs/promises';
import { join } from 'node:path';
import { homedir } from 'node:os';

export const resolveCodexHome = (value?: string) => value || process.env.CODEX_HOME || join(homedir(), '.codex');
import { DatabaseSync } from 'node:sqlite';

const dayOf = (value: string | Date) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date(value));
const shiftDay = (day: string, n: number) => new Date(Date.parse(`${day}T12:00:00Z`) + n * 86400000).toISOString().slice(0, 10);
...
export function parseCodexUsageRecord(line: string): UsageRecord | null {
...
export async function readCodexTokenTrend({ codexHome, dataDir, now = new Date(), days = 14, afterDay = null, maxBytes = 32 * 1024 * 1024, maxMs = 1000 }: {codexHome?: string; dataDir: string; now?: Date; days?: number; afterDay?: string | null; maxBytes?: number; maxMs?: number}) {
  ...
  try {
    ...
    for (const path of files) {
      ...
      const prior = lookup.get(path) as unknown as FileRecord | undefined;
            ...
            try {
              for (const line of parts) {
                ...
                if (record) {
                  ...
                  if (previous && (['thread', 'turn', 'day', 'input', 'cached', 'cache_write', 'output', 'total'] as const).some(key => previous[key] !== record[key])) warn.run('conflicting-response');
    ...
    const rows = db.prepare('SELECT * FROM responses WHERE day>=? AND day<=?').all(first, today) as unknown as UsageRecord[];
