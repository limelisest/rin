...
export class CodexBridge extends CodexQueue {
  onEvent: (event: CodexEvent) => void; getCursor?: (key: string) => unknown; setCursor?: (key: string, value: unknown) => void; pollMs: number; watchers: Map<string, Watcher>; appIpc: CodexAppIpc | null; wakeApp: BridgeOptions['wakeApp'] | null; wakeTimeoutMs: number;
