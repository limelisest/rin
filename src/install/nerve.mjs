...
async function availablePort() {
  ...
  await new Promise((accept,reject)=>{server.once('error',reject);server.listen({host:'127.0.0.1',port:0,exclusive:true},accept);});
...
export async function ensureNerveMcp({home,codexHome,node=process.execPath,binary,command,run=coreRun,writeConfig}={}) {
  ...
  let current;
  if(inspection.code===0)current=object(JSON.parse(inspection.stdout),'Nerve MCP entry');
  ...
  const daemon=object(await readOptional(daemonPath) ?? {chat:null,nerve:null},'daemon configuration');
  ...
  object(config,'Nerve configuration');
  ...
  if(config.port!==undefined && (!Number.isInteger(config.port) || config.port<1 || config.port>65535))throw new Error('Invalid Nerve port');
  ...
  const edits=[];
  const edit=(key,value)=>edits.push({keyPath:`mcp_servers.nerve.${key}`,value,mergeStrategy:'upsert'});
