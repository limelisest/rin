...
export async function findNpmCli({ node = process.execPath, platform = process.platform, env = process.env } = {}) {
  ...
  for (const candidate of candidates.filter((value): value is string => Boolean(value))) {
    ...
    catch (e) { if (errorCode(e) !== 'ENOENT' && errorCode(e) !== 'ENOTDIR') throw e; }
  }
...
export async function run(command: string, args: string[], options: ExecOptions = {}): Promise<ExecResult> {
  return new Promise<ExecResult>((accept, reject) => {
...
export async function readInstall(home: string): Promise<InstallState> {
...
export async function withInstallLock<T>(home: string, fn: ()=>Promise<T>): Promise<T> {
  ...
  try { await mkdir(lock); } catch (e) { if (errorCode(e) === 'EEXIST') throw new Error('Another install/update may be running. Remove install.lock only after checking that process.'); throw e; }
...
export async function switchRelease(home: string, candidate: Pick<Candidate,'sha'>, state: InstallState | null, service: ManagedService): Promise<InstallState> {
  const running = state ? await service.isRunning() : false;
  if (running) await service.stop();
  const next: InstallState = { ...state, schema: 1, type: 'git', repository: state?.repository || REPOSITORY, current: candidate.sha, previous: state?.current || null, node: process.execPath };
  try {
