...
function run(command: string[], args: string[], env: NodeJS.ProcessEnv, { timeoutMs, children }: {timeoutMs:number; children:Set<ChildProcess>}) {
  return new Promise<string>((resolve, reject) => {
    ...
    const append = (current: string, chunk: string) => (current + chunk).slice(-MAX_COMMAND_OUTPUT);
...
function messageIdFrom(output: string): string | undefined {
...
export class CodexQueue {
 declare command: string[]; declare codexHome?: string; declare queueTimeoutMs: number; declare started: boolean; declare children: Set<ChildProcess>;
  ...
  async queue(threadId: string, { text, files = [] }: MessageInput = {}) {
    ...
    if (images.length) {
      const error: NodeJS.ErrnoException = new Error('Codex queue does not support image attachments; an available App owner is required');
