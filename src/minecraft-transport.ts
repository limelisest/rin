...
import type {MinecraftMessage, MinecraftTask, MinecraftSend, MinecraftReceipt, MinecraftState, MinecraftResponse, MinecraftOptions} from './minecraft-types.js';
...
const syncDirectory = async (path: string) => { const dir = await open(path, 'r'); try { await dir.sync(); } finally { await dir.close(); } };
...
function validateMessage(raw: MinecraftMessage, lock?: MinecraftSource) {
...
function validateTask(task?: MinecraftTask) {
...
export class MinecraftTransport {
 declare endpoint:URL; declare secret:string; declare path:string; declare lockPath:string; declare source:MinecraftSource; declare enqueue:MinecraftOptions['enqueue']; declare fetch:typeof fetch; declare timeoutMs:number; declare token:string; declare writeTail:Promise<void>; declare operationTail:Promise<unknown>; declare lock?:FileHandle; declare state:MinecraftState;
  constructor({ endpoint, secret, stateFile, source, enqueue, fetchImpl = fetch, timeoutMs = 10000 }: MinecraftOptions) {
  ...
  static async inspectLock(stateFile: string) {
    try {
      ...
      if (Number.isInteger(metadata.pid) && metadata.pid > 0) {
        ...
        catch (error) { processAlive = (error as NodeJS.ErrnoException).code === 'EPERM' ? true : false; }
      }
      return {exists:true,metadata,processAlive};
    } catch (error) { if ((error as NodeJS.ErrnoException)?.code === 'ENOENT') return {exists:false}; throw new Error('Invalid Minecraft transport lock'); }
  }
  static async recoverStaleLock(stateFile: string, {minimumAgeMs=60000,nowMs=Date.now()} = {}) {
  ...
  async open() {
    ...
    try { this.lock = await open(this.lockPath, 'wx', 0o600); } catch (error) { if ((error as NodeJS.ErrnoException)?.code === 'EEXIST') throw new Error('Minecraft transport is already running; inspect its lock before recovery'); throw error; }
    try {
      ...
      try { this.state = JSON.parse(await readFile(this.path, 'utf8')); } catch (error) { if ((error as NodeJS.ErrnoException)?.code !== 'ENOENT') throw error; this.state = {version:1,pages:[],inbox:{},outbox:{}}; await this.save(); }
  ...
  async close() {
    ...
    try { const lock = JSON.parse(await readFile(this.lockPath, 'utf8')); if (lock.token === this.token) { await unlink(this.lockPath); await syncDirectory(dirname(this.path)); } } catch (error) { if ((error as NodeJS.ErrnoException)?.code !== 'ENOENT') throw error; }
  }
  async request(path: string, options: RequestInit = {}): Promise<MinecraftResponse | undefined> {
    ...
    let body: MinecraftResponse | undefined; try { body = await response.json() as MinecraftResponse; } catch {}
  ...
  async send({id,messageId,kind,text:replyText,task}: MinecraftSend) {
    return this.exclusive(async () => {
      ...
      try {
        ...
        this.state.outbox[id].result = result as MinecraftReceipt;
        await this.save();
        return result;
      } catch {
