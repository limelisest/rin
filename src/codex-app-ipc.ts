import type { LocalFile, MessageInput, AppReceipt, IpcResponse } from './runtime-types.js';
...
async function messageParts(text: string | undefined, files: LocalFile[]) {
...
function uncertain(reason: string) {
  const error: NodeJS.ErrnoException = new Error(`Codex App IPC ${reason}; outcome uncertain`);
  error.code = 'CODEX_APP_IPC_UNCERTAIN';
  return error;
}

function ordinary(reason: string) {
  const error: NodeJS.ErrnoException = new Error(`Codex App IPC ${reason}`);
  error.code = 'CODEX_APP_IPC_ERROR';
  return error;
}

async function verifySocket(socketPath: string) {
...
export class CodexAppIpc {
 declare codexHome: string; declare timeoutMs: number; declare stopped: boolean; declare operations: Set<{socket: net.Socket; cancel: () => void}>;
  constructor({ codexHome, timeoutMs = 30_000 }: {codexHome?: string; timeoutMs?: number} = {}) {
  ...
  async steer(threadId: string, { text, cwd, files = [], start = false }: MessageInput = {}) {
    ...
    return new Promise<AppReceipt | null>((resolve, reject) => {
      const socket = net.createConnection(socketPath);
      const pending = new Map<string, {resolve:(response:IpcResponse)=>void; reject:(error: unknown)=>void; timer:NodeJS.Timeout}>();
      ...
      const finish = (error: unknown, value?: AppReceipt | null) => {
        if (settled) return;
        settled = true;
      ...
      const transportFailure = (reason: string) => fail(mutationSent ? uncertain(reason) : ordinary(reason));

      const request = (method: string, params: unknown, { version, targetClientId, mutation = false }: {version:number; targetClientId?:string; mutation?:boolean}) => new Promise<IpcResponse>((requestResolve, requestReject) => {
      ...
      socket.once('error', error => {
        if (settled) return;
        if (!connected && !mutationSent && ['ENOENT', 'ECONNREFUSED'].includes((error as NodeJS.ErrnoException)?.code || '')) return finish(null, null);
      ...
      socket.once('connect', async () => {
        ...
        try {
          ...
          fail(mutationSent && (error as NodeJS.ErrnoException)?.code !== 'CODEX_APP_IPC_UNCERTAIN'
