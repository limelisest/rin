...
import { exists, run } from './core.js';

const present = async (file: string) => { try { await lstat(file); return true; } catch (e) { if (errorCode(e) === 'ENOENT') return false; throw e; } };
const normalized = (value: string, platform: NodeJS.Platform) => platform === 'win32' ? value.replaceAll('\\', '/').toLowerCase() : value;
async function recordAt(file: string, { inaccessibleIsMissing = false } = {}): Promise<LegacyRecord | null | undefined> {
  try { return JSON.parse(await readFile(file, 'utf8')); }
  catch (e) {
    if (errorCode(e) === 'ENOENT') return null;
    if (inaccessibleIsMissing && (errorCode(e) === 'EACCES' || errorCode(e) === 'EPERM')) return undefined;
...
export async function inspectLegacy({ userHome = homedir(), env = process.env, platform = process.platform, binDir = join(userHome, '.local/bin') } = {}) {
  ...
  for (const directory of new Set([binDir, join(userHome, '.local/bin'), ...(env.PATH || env.Path || '').split(platform === 'win32' ? ';' : ':')].filter(Boolean).map(path => resolve(path)))) {
    for (const name of names) {
      ...
      } catch (e) { if (errorCode(e) !== 'ENOENT' && errorCode(e) !== 'EISDIR') throw e; }
    }
  }
...
export async function disableLegacy(legacy: LegacyInstallation | null, { exec = run, platform = process.platform, uid = process.getuid?.() } = {}) {
  if (!legacy) return;
  ...
  if (service?.kind === 'launchd') {
    ...
    if (!unit || !/^rin-daemon-[a-zA-Z0-9_.@-]+\.service$/.test(unit || '')) throw new Error('Unrecognized legacy systemd unit; inspect it before migration');
...
export interface LegacyService {kind: string; label?: string; unit?: string; name?: string; path?: string; servicePath?: string}
interface LegacyRecord {installDir?: string; defaultInstallDir?: string; targetUser?: string; defaultTargetUser?: string; service?: LegacyService}
export interface LegacyInstallation {root: string; service?: LegacyService; cli: string[]; records: string[]; inaccessibleRoot: boolean}
