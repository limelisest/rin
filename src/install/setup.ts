...
import { installHome, REPOSITORY, exists, run, withInstallLock, prepareRelease, atomicJSON, codexCommand } from './core.js';
import { createService } from './service.js';
import { installProducts, installHistoryTool, historySupport } from './products.js';
import { applyRecommendedCodexProfile } from './profile.js';
import {appendAgentsInstructions} from './instructions.js';
import {collectChoices,runSetupProgress,setupUI} from './setup-ui.js';
import { inspectLegacy, disableLegacy } from './legacy.js';
...
export async function writeNerveLauncher(home: string) {
...
export async function writeLaunchers(home: string, { binDir, node = process.execPath, platform = process.platform, publish = true }: {binDir: string; node?: string; platform?: NodeJS.Platform; publish?: boolean}) {
...
export async function setup({ home = installHome(), repository = REPOSITORY, binDir = join(homedir(), '.local/bin'), codexHome = process.env.CODEX_HOME || join(homedir(), '.codex') } = {}) {
  ...
  return await withInstallLock(home, async () => {
    ...
    try {
      ...
      throw new Error(`Installation cutover failed: ${errorMessage(error)}. Legacy CLI launchers were restored; its service remains stopped.`);
    }
