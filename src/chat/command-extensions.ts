import type { ChatCommand, Logger } from './types.js';
...
function warn(log: Logger) {
  try { log?.warn?.(WARNING); } catch {}
}

function descriptor(input: unknown): ChatCommand | null {
  ...
  return {
    ...
    run: value.run as ChatCommand['run'],
  };
}

export async function loadCommandExtensions({ directory, reservedNames = [], log = console }: {directory?: string; reservedNames?: string[]; log?: Logger} = {}) {
