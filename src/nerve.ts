...
import type {NerveConfig, NerveEvent, NerveTrigger, TriggerRow} from './nerve-types.js';
...
import { CodexAppExec } from './codex-app-exec.js';
import { AttentionService } from './attention-service.js';
import { MinecraftTransport } from './minecraft-transport.js';

const digest = (value: string) => createHash('sha256').update(value).digest('hex');
const json = (value: unknown) => JSON.stringify(value);
const canonical = (value: unknown) => JSON.stringify(value, (_, v) => v && typeof v === 'object' && !Array.isArray(v) ? Object.fromEntries(Object.entries(v).sort(([a],[b])=>a.localeCompare(b))) : v);
...
export class Store {
 declare db: DatabaseSync;
  ...
  triggers(): NerveTrigger[] { return this.db.prepare('SELECT * FROM triggers ORDER BY id').all().map(r=>({...JSON.parse(r.definition as string) as NerveTrigger,revision:r.revision as number,managed:true})); }
  upsertTrigger(trigger: NerveTrigger) {
    const definition=canonical(trigger),old=this.db.prepare('SELECT * FROM triggers WHERE id=?').get(trigger.id) as unknown as TriggerRow | undefined;
...
export function scheduleSlot(trigger: NerveTrigger, now = Date.now()) {
...
export class Nerve {
 declare config:NerveConfig; declare store:Store; declare busy:boolean; declare scanning:boolean; declare codex:CodexExec|CodexAppExec|null; declare running:Set<Promise<void>>; declare serialRunning:boolean; declare attention:AttentionService|null; declare minecraftSecret?:string; declare minecraft?:MinecraftTransport; declare minecraftSync?:Promise<unknown>; declare stopping?:boolean;
  constructor(config: NerveConfig, store: Store, { minecraftSecret }: {minecraftSecret?:string} = {}) {
  ...
  triggers() {
    const all=new Map<string,NerveTrigger>((this.config.triggers || []).map(t=>[t.id,{...t,managed:false}]));
    for(const t of this.store.triggers())all.set(t.id,t);
    return [...all.values()];
  }
  upsertTrigger(input: NerveTrigger) {
    const allowed=['id','target','at','daily','everySeconds','timeZone','payload','check','timeoutMs','enabled'];
    const trigger=Object.fromEntries(Object.entries(input).filter(([k])=>allowed.includes(k))) as unknown as NerveTrigger;
  ...
  personaActive() {
    ...
    return Boolean((this.codex as CodexAppExec).bridge.activeThread(target.threadId!));
  }
  async scan(now = Date.now()) {
    for (const trigger of this.triggers()) {
      ...
      try {
        ...
        } catch(e: unknown) { this.store.db.exec('ROLLBACK'); throw e; }
      } catch(e: unknown) { log('check_failed',{trigger:trigger.id,error:(e as Error).message}); }
    }
  }
...
export function makeServer(nerve: Nerve, token: string | undefined) {
  ...
  return createServer(async (req,res) => {
    ...
    }catch(e: unknown){reply(400,{error:(e as Error).message});}
  });
}
