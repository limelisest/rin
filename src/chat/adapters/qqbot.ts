import type { QQBot, QQBotInboundMessage, QQBotOptions, QQBotEventMap, ReplyTarget, Logger as QQLogger } from '@tencent-connect/qqbot-nodejs';
import type { Logger } from '../types.js';
type QQClient = Pick<QQBot, 'on' | 'off' | 'start' | 'stop' | 'sendText' | 'sendImage' | 'sendVoice' | 'sendVideo' | 'sendFile' | 'sendTyping' | 'api' | 'tokenManager' | 'apiClient'>;
interface QQConfig extends AdapterConfig {
 appId: string; appSecret: string; attachmentTimeoutMs?: number; fetch?: typeof fetch;
 sdk?: {QQBot: new (options: QQBotOptions) => QQClient}; bot?: QQClient;
 transport?: 'websocket' | 'webhook'; port?: number; path?: string; intents?: number;
}
...
import { syncQQCommandPanels } from '../qq-commands.js';
...
function logger(log: Partial<Logger> & {debug?: (...args: unknown[]) => void} = {}): QQLogger {
  return {
    debug: (...args: unknown[]) => {
...
async function downloadAttachments(msg: QQBotInboundMessage, config: QQConfig, context: AdapterContext) {
...
export function createAdapter(config: QQConfig, context: AdapterContext) {
  ...
  let bot: QQClient | undefined;
  let startPromise: Promise<void> | undefined;
  let handler: QQBotEventMap["message"] | undefined;
  const commands = Array.isArray(context.commands) ? context.commands : COMMANDS;

  function targetFor(target: ChatTarget): ReplyTarget {
    return {
  ...
  return {
    ...
    async send(target: ChatTarget, output: ChatOutput): Promise<{id: string}> {
      ...
      for (const file of output.files || []) {
        ...
        catch (errorValue) { const error = platformError(errorValue); if(explicitMediaRejection(error))error.fallbackSafe=true;throw error; }
        result ||= media?.message;
      }
      if (!result) throw new Error('QQ official send requires text or files');
      const id = result.id || ('message_id' in result ? result.message_id : undefined);
      if (id == null || id === '') throw new Error('QQ official send returned no message id');
      return { id: String(id) };
    },
    async typing(target: ChatTarget) {
      if (!bot) throw new Error('QQ official adapter is not started');
