...
import type { Logger, AttentionRecord } from './types.js';
...
export class AttentionClient {
  url: string; token: string; store: ChatStore; fetch: typeof fetch; log: Logger; busy: boolean; stopped: boolean; abort: AbortController;
  constructor(configPath: string, store: ChatStore, {fetchImpl=globalThis.fetch, log=console}: {fetchImpl?: typeof fetch; log?: Logger}={}) {
  ...
  observe(record: AttentionRecord) {
  ...
  async flush() {
    ...
    try {
      for(const row of this.store.db.prepare("SELECT id,payload FROM attention_outbox WHERE state='pending' ORDER BY created LIMIT 100").all()) {
        ...
        const response=await this.fetch(this.url,{method:'POST',headers:{authorization:`Bearer ${this.token}`,'content-type':'application/json'},body:String(row.payload),redirect:'error',signal:AbortSignal.any([this.abort.signal,AbortSignal.timeout(10000)])});
